<template>
  <div>
    ...
    <div v-if="isMobileFilter" class="flex justify-center">
      ...
      <div
        class="absolute top-[70px] z-10 bg-white w-full shadow transition-all duration-300 delay-300"
...
const { sortedItems, initialSorting } = storeToRefs(generalStore);
...
const filterMobile = ref<Filter | null>(null);
const refDesktopFilter = ref([]);
const filterArray = ref([]);

const handleDesktopFilterItems = (filter: Filter) => {
  if (filter.isOpen) {
    filter.isOpen = false;
    return;
  }
...
const handleMobileFilterItems = (item: Filter) => {
...
const handleSelect = (item: { name: string; isSelected: boolean }, filter: Filter) => {
  item.isSelected = !item.isSelected;
  prepareFilterArray(item, filter);

  if (filterArray.value.length === 0) {
    sortedItems.value = initialSorting.value;
    return;
  }

  sortedItems.value = initialSorting.value.filter((arr1Item) => {
...
const prepareFilterArray = (item: { name: string; isSelected: boolean }, filter: Filter) => {
...
const updateWidth = () => {
  isMobileFilter.value = window.innerWidth < 1024;
  isDesktopFilter.value = window.innerWidth >= 1024;
};
